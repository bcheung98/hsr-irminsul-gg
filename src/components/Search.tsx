...
type Category = "Characters" | "Light Cones" | "Relics";
...
function Search() {
    ...
    const relics = [...cavernRelics, ...planarRelics].sort((a, b) =>
        a.displayName.localeCompare(b.displayName)
    );
    ...
    const data: Data[] = [
        ...
        ...weapons.map((wep) => ({
            ...
            category: "Light Cones" as Category,
        })),
        ...relics.map((relic) => ({
            name: relic.name,
            displayName: relic.displayName,
            category: "Relics" as Category,
        })),
    ];
...
function getImageIcon(category: Category, name: string) {
    switch (category) {
        ...
        case "Light Cones":
            return `lightcones/small/${name}`;
        case "Relics":
            return `relics/sets/${name.split(" ").join("_")}/_icon`;
    }
}
